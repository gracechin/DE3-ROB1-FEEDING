...
class image_converter:
  ...
  def __init__(self):
    ...
    self.tss = message_filters.TimeSynchronizer((Subscriber("/camera/depth_registered/image_raw", Image,queue_size =1)),
                                           (Subscriber("/camera/rgb/image_rect_color",Image,queue_size =1)))
    tss.registerCallback(callback_d)

  def callback_d(self,depth,img):

    try:
      depth_image_raw = self.bridge.imgmsg_to_cv2(img, "passthrough")
      depth_image = (100*depth_image_raw).astype(np.uint8)
    except CvBridgeError as e:
      print(e)
    try:
      cv_image = self.bridge.imgmsg_to_cv2(depth, "bgr8")
    except CvBridgeError as e:
      print(e)
